...
The original implementation was validated on 2026-09-25 with Java 11 and RuneLite 1.12.39 (resolved from `latest.release`), including rendered Ancients, Standard, normal, custom-text, hover and flashing previews. Version 1.2.1 passes **50 tests** and builds successfully, with regressions for independent book/pouch banking, multiple updates before one tick, stale confirmations, snapshot read counts and maximum rune stacks. No live game session was used for automated validation.
